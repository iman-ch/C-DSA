...
static BOOLEAN avl_remove_aux(avl_linked *source, avl_node **node, const data_ptr key, data_ptr item) {
    ...
    if (comp < 0) {
        ...
        if ((*node)->left == NULL || (*node)->right == NULL) { // 0 or 1 child
            ...
            data_ptr tempItem = (*node)->item;
            (*node)->item = (*successor)->item;
            (*successor)->item = tempItem;

            if ((*successor)->right != NULL) { // if successor has a right child, link this child to successor's parent
                avl_node *tempSuccessor = *successor;
                *successor = (*successor)->right;
                free(tempSuccessor);
             } else { // successor has no right child, remove
                free(*successor);
                *successor = NULL;
            }
            source->count--;
