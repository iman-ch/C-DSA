...
void bst_free_aux(bst_node *node) {
    if (node == NULL)
        return;
...
BOOLEAN bst_empty(const bst_linked *source) {
    return (source->root == NULL) ? TRUE : FALSE;
}

BOOLEAN bst_full_aux(const bst_node *node){
    if (node == NULL) return TRUE;
    if (node->left == NULL && node->right == NULL) return TRUE;
    if (node->left != NULL && node->right != NULL){
        return bst_full_aux(node->left) && bst_full_aux(node->right);
    }
    return FALSE;
}
...
int bst_count_aux(const bst_node *node) {
    if (node == NULL) return 0;
    int l = bst_count_aux(node->left);
    int r = bst_count_aux(node->right);
    return 1 + l + r;
}

// Returns number of items in a BST.
int bst_count(const bst_linked *source) {
    return bst_count_aux(source->root);
}

void bst_inorder_aux(const bst_node *node, data_ptr *items, int *index) {
    if (node==NULL) return;
...
void bst_preorder_aux(const bst_node *node, data_ptr *items, int *index) {
    if (node==NULL) return;
    items[*index]= node->item;
    (*index)++;
...
void bst_postorder_aux(const bst_node *node, data_ptr *items, int *index) {
    if (node==NULL) return;
...
BOOLEAN bst_retrieve(bst_linked *source, const data_ptr key, data_ptr item) {
    ...
    while (curr){
        ...
        else if (data_compare(curr->item, key) > 0) curr = curr->left;
...
int bst_leaf_count_aux(const bst_node *node) {
    ...
    else return bst_leaf_count_aux(node->left) + bst_leaf_count_aux(node->right);
...
BOOLEAN bst_balanced_aux(const bst_node *node) {
    if (node == NULL) return TRUE;
    int lh = bst_node_height(node->left);
    int rh = bst_node_height(node->right);
        if (abs(lh-rh) <= 1 && bst_balanced_aux(node->left) && bst_balanced_aux(node->right)) {
            return TRUE;
        }
    return FALSE;
}
...
BOOLEAN bst_balanced(const bst_linked *source) {
    if (source == NULL || source->root == NULL) return TRUE;
    else return bst_balanced_aux(source->root);
}
